...
int FindNearestPromoter(string, int,string&,int&);
private:
...
int PromoterClass::FindNearestPromoter(string i_chr, int pos,string& nearestgenename, int& nearestgeneTSS){

	int chr_index;
	i_chr.erase(i_chr.find_last_not_of(" \n\r\t")+1);
	for(int k = 0;k < ChrNames_proms.size(); ++k){
		if(ChrNames_proms[k].compare(i_chr.c_str()) == 0){ // Find the right chromosome (genes are indexed acc. to which chromosome they are on
			chr_index = k;
			break;
		}
	}
	int tempdist = 2000000000, distance;
	for(int i = 0; i < prom_indexes[chr_index].size(); ++i){ //Iterate over all refseq genes on that chromosome
		distance = abs(pos - proms[prom_indexes[chr_index][i]].TSS);
		if(distance <= tempdist){
			tempdist = distance;
			nearestgenename = proms[prom_indexes[chr_index][i]].genes[0];
			nearestgeneTSS = proms[prom_indexes[chr_index][i]].TSS;
