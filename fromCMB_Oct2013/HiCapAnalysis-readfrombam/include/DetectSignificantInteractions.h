...
//	double gc1, gc2;
	...
	FileName.append(BaseFileName);
	FileName.append("_NofINTPerBin_PE_");
	FileName.append(".txt");
	ofstream outf1(FileName.c_str());

	FileName2.append(BaseFileName);
	FileName2.append("_ClusterDetails_PE_");
	FileName2.append(".txt");
	ofstream outf2(FileName2.c_str());

	FileName3.append(BaseFileName);
	FileName3.append("_NormalisedReadCounts_PE");
	...
	for(int k=0;k<prs.refseq[i].isoformpromotercoords.size();++k){
		for(int j=1;j<NumberofBins;++j){
			if(j < 98 || j > 102){ // do not look for the neighboring 10 kb upstream and downstream
				...
				q=alglib::normaldistribution(z_score);
				p_value=1-q;
				if((p_value<SignificanceThreshold) && prs.refseq[i].AllExperiments_IntBins[whichexperiment].interactorbins[k][j] >= MinNumberofPairs){P
					int bincoordst = 0;
					...
					bool pp = prs.AnnotateWithPromoters(prs.refseq[i].chr, bincoordst);
					if (!pp){
...
*/
	...
	FileName2.append("IntScore_PeakOverlap.txt");
	ofstream outf2(FileName2.c_str());

	FileName3.append(BaseFileName);
	FileName3.append(abname);
	FileName3.append("IntScore_NoPeakOverlap.txt");
	ofstream outf3(FileName3.c_str());
