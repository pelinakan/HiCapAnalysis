...
public :
	...
	double GetMappability(string, int, int); // chr, start, length of the region
private:
...
void MappabilityClass::InitialiseVars(void){
	span = 200;
	NumberofBits;  // = window/span; // number of bits to reads
	lowerLimit = 0.0;
	dataRange = 1.0;
	...
	s.append(dirname);
	s.append("Mappability.36mers.offsets_mm9.binary");
	ifstream Mapptextbin(s.c_str(), ios::binary);
	s.clear();

#endif
#ifdef WINDOWS
	ifstream RESitesf("C:\\WORK\\3c-SeqCap\\CODES\\3C_Analysis\\HiCapAnalysis\\SupplementaryFiles\\Mappability.36mers.mm9.chr_offsets.txt");
	ifstream Mapptextbin("C:\\WORK\\3c-SeqCap\\CODES\\3C_Analysis\\HiCapAnalysis\\SupplementaryFiles\\Mappability.36mers.offsets_mm9.binary", ios::binary);
#endif
...
#endif
#ifdef WINDOWS
	ifstream file2("C:\\WORK\\3c-SeqCap\\CODES\\3C_Analysis\\HiCapAnalysis\\SupplementaryFiles\\Mappability.36mers.offsets_mm9.binary", ios::binary);
#endif

// mappability_mm9.binary file contains start, end count and offset information
...
double MappabilityClass::GetMappability(string chr, int coord, int rlength){ //rlength = region length

#ifdef UNIX
	string s;
	s.append(dirname);
	s.append("MM9.Mappability.36mer.bin");
	ifstream file(s.c_str(), ios::in | ios::binary | ios::ate);
	s.clear();

#endif
#ifdef WINDOWS
	ifstream file ("C:\\WORK\\3c-SeqCap\\CODES\\3C_Analysis\\HiCapAnalysis\\SupplementaryFiles\\MM9.Mappability.36mer.bin", ios::in | ios::binary | ios::ate);
#endif

NumberofBits = rlength / span; // How many bits to read
